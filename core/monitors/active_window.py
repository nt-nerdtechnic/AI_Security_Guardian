...
class ActiveWindowMonitor(threading.Thread):
    ...
    def take_snapshot(self, trigger_name):
        ...
        try:
            ...
            msg = (
                i18n.get(
                    "visual_alert", trigger_name=trigger_name, filename=filepath.name
                )
                if i18n
                else f"Visual alert: {trigger_name}"
            )
            logger.warning(msg)
    ...
    def run(self):
        logger.info(
            i18n.get("visual_monitor_started") if i18n else "Visual monitor started"
        )
        while self.running:
            try:
                ...
                if app_name != self.last_app:
                    if app_name:
                        for target in self.target_windows:
                            if (
                                target.lower() in app_name.lower()
                                or target.lower() in window_title.lower()
                            ):
