...
class NetworkMonitor(threading.Thread):
    def __init__(self, config, notifier, mitigator=None):
        ...
        self.threshold = (
            self.config.get("high_bandwidth_threshold_mb", 50) * 1024 * 1024
        )
        self.suspicious_ports = self.config.get("suspicious_ports", [])

    def run(self):
        logger.info(
            i18n.get("network_monitor_started") if i18n else "Network monitor started"
        )
        while self.running:
            try:
                output = subprocess.check_output(
                    ["lsof", "-i", "-nP"], stderr=subprocess.STDOUT
                ).decode()
                            ...
                            if ":" in remote_addr:
                                ...
                                try:
                                    ...
                                    if port in self.suspicious_ports:
                                        ...
                                        self.notifier.send_interactive_alert(
                                            msg,
                                            [
                                                [
                                                    {
                                                        "text": "🛡️ Quarantine",
                                                        "callback_data": f"quarantine|{parts[8]}",
                                                    },
                                                    {
                                                        "text": "🛑 Terminate",
                                                        "callback_data": f"terminate|{pid}",
                                                    },
                                                ],
                                                [
                                                    {
                                                        "text": "🙈 Ignore",
                                                        "callback_data": f"ignore|{pid}",
                                                    }
                                                ],
                                            ],
                                        )
