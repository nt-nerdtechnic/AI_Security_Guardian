...
class ClipboardMonitor(threading.Thread):
    ...
    def run(self):
        logger.info(
            i18n.get("clipboard_monitor_started")
            if i18n
            else "Clipboard Monitor Started"
        )
        while self.running:
            try:
                ...
                logger.error(
                    i18n.get("clipboard_monitor_error", error=e)
                    if i18n
                    else f"Clipboard error: {e}"
                )
    ...
    def check_content(self, content):
        ...
        for rule_name, pattern in self.rules.items():
            if re.search(pattern, content):
                ...
                msg = (
                    i18n.get("clipboard_alert", rule_name=rule_name)
                    if i18n
                    else f"Regex triggered: {rule_name}"
                )
                logger.warning(msg)
