...
class SystemResourceMonitor(threading.Thread):
    ...
    def run(self):
        logger.info(
            i18n.get("system_resource_monitor_started")
            if i18n
            else "System Resource Monitor started"
        )
        ...
        while self.running:
            try:
                ...
                if cpu_percent >= self.cpu_threshold:
                    msg = (
                        i18n.get("resource_cpu_high", cpu=cpu_percent)
                        if i18n
                        else f"High CPU: {cpu_percent}%"
                    )
                ...
                if mem.percent >= self.ram_threshold:
                    msg = (
                        i18n.get("resource_ram_high", ram=mem.percent)
                        if i18n
                        else f"High RAM: {mem.percent}%"
                    )
                ...
                if disk.percent >= self.disk_threshold:
                    msg = (
                        i18n.get("resource_disk_high", disk=disk.percent)
                        if i18n
                        else f"High Disk: {disk.percent}%"
                    )
                    self._alert_and_log("Disk", msg, disk.percent)

            except Exception as e:
                err_msg = (
                    i18n.get("system_resource_monitor_error", error=e)
                    if i18n
                    else f"Error in monitor: {e}"
                )
