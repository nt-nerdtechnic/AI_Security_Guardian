...
class SystemHeartbeat(threading.Thread):
    ...
    def run(self):
        ...
        while self.running:
            active_modules = [
                m.__class__.__name__ for m in self.monitors if m.is_alive()
            ]
            ...
            if mem_mb > MAX_MEM_MB:
                msg = (
                    i18n.get("mem_leak_shutdown", mem=round(mem_mb, 1))
                    if i18n
                    else f"Memory leak detected: {round(mem_mb,1)}MB"
                )
                logger.critical(msg)
                os._exit(1)
            ...
            if cpu_usage > MAX_CPU_PERCENT:
                msg = (
                    i18n.get("resource_cpu_high", cpu=cpu_usage)
                    if i18n
                    else f"High CPU: {cpu_usage}%"
                )
                logger.warning(msg)
