...
class KeystrokeMonitor(threading.Thread):
        ...
        try:
            if hasattr(key, 'char') and key.char is not None:
                ...
                for kw in self.high_risk_keywords:
                    if kw in cmd:
                        msg = i18n.get('terminal_alert', command=cmd, keyword=kw) if i18n else f"Regex triggered: {kw}"
                        logger.warning(msg)
    ...
    def run(self):
        logger.info(i18n.get('terminal_monitor_started') if i18n else "Terminal Monitor started")
