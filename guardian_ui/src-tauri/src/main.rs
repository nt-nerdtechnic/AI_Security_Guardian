...
fn get_incident_stats() -> IncidentStats {
    // Relative to the executable in src-tauri/target/debug or release
    // In dev, the working dir is often src-tauri.
    ...
    for line in content.lines() {
        ...
        // Handle the initial seed array if present
...
fn get_config() -> Result<GuardianConfig, String> {
    ...
    // Simple manual parsing to avoid adding heavy yaml dependency in this sprint
...
fn update_config(mode: String, modules: ModulesConfig) -> Result<(), String> {
    ...
    for line in existing_content.lines() {
        ...
        if trimmed.starts_with("mode:") {
            ...
            in_modules = false; // end of modules section in current schema
        } else {
            new_lines.push(line.to_string());
        }
    }
    ...
    // Signal the background process (guardian.py) to reload
